...
ALTER TABLE my_set_exercises ADD COLUMN IF NOT EXISTS reps_low     INTEGER;
ALTER TABLE my_set_exercises ADD COLUMN IF NOT EXISTS muscle_groups TEXT;
...
ALTER TABLE session_exercises ADD COLUMN IF NOT EXISTS load_mode     VARCHAR(4) DEFAULT 'high';
ALTER TABLE session_exercises ADD COLUMN IF NOT EXISTS low_load_pct  NUMERIC(5,1) DEFAULT 30;
...
-- ルーマニアンデッドリフトを登録
INSERT INTO exercises (name, body_part, needs_bench, primary_muscle, location)
VALUES ('ルーマニアンデッドリフト', '下肢', false, 'ハムストリングス', 'gym')
ON CONFLICT (name) DO UPDATE SET body_part='下肢', needs_bench=false, primary_muscle='ハムストリングス', location='gym';
